...
from optimization.model_decouple import LinearModel, AffineModel
...
class AdaptiveFitting(BaseOptimizer):
    def __init__(self, mlp_mapping, z1, z2, hidden_dim, lamda, z1_val=None, z2_val=None, learning_rate=0.001, dropout=0.3):
        """
        AdaptiveFitting trains a bayesian linear model and based on the uncertainty, chooses either a linear or MLP mapping.

        Parameters:
            mlp_mapping (NeuralNetworkFitting): Pretrained instance of the MLP class
    ...
    def fit(self):
        ...
        self.linear_model = BayesianLinearRegression.load_from_checkpoint(trainer.checkpoint_callback.best_model_path, source_dim=self.latent_dim1, target_dim=self.latent_dim2, learning_rate=self.linear_model.learning_rate, lamda=self.linear_model.lamda)
    ...
    def transform(self, z1) -> torch.Tensor:
        ...
        z2_mlp = self.mlp_model.transform(z1).detach().cpu()
        # for each sample, if the variance of the linear model is greater than the 90th percentile, use the mlp model
        mask = (var_linear < torch.quantile(var_linear, 0.9)).unsqueeze(1).expand_as(z2_linear)
        z2 = torch.where(mask, z2_linear, z2_mlp)
        # z2 = z2_linear
        return z2

    @classmethod
    def from_file(cls, path, mlp_mapping=None):
        """
        Loads the results of the optimization problem from a file.
        ...
        instance = cls(mlp_mapping, np.empty((1, 1)), np.empty((1, 1)), 0, 0)
        instance.linear_model = linear_model
        return instance


class HybridFitting(BaseOptimizer):
    def __init__(self, linear_model, z1, z2, hidden_dim, lamda, z1_val=None, z2_val=None, learning_rate=0.001, dropout=0.3, noise_sigma=0.1):
        """Uses cvxpy linear mapping along with MLP mapping to predict the residuals.

        Parameters:
            linear_model (LinearFitting): Pretrained instance of the LinearFitting class
            z1 (np.ndarray): Input data matrix of shape (n_samples, latent_size)
            z2 (np.ndarray): Output data matrix of shape (n_samples, latent_size)
            hidden_dim (int): Number of neurons in the hidden layer
            lamda (float): Regularization parameter
        ...
        self.z1 = torch.from_numpy(z1) if isinstance(z1, np.ndarray) else z1
        self.z2 = z2.detach().cpu().numpy() if isinstance(z2, torch.Tensor) else z2

        if z1_val is not None:
            self.z1_val = torch.from_numpy(z1_val) if isinstance(z1_val, np.ndarray) else z1_val
            self.z2_val = z2_val.detach().cpu().numpy() if isinstance(z2_val, torch.Tensor) else z2_val

    def fit(self):
        """
        Solves the optimization problem.
        """
        residuals = self.z2 - self.linear_model.transform(self.z1).detach().cpu().numpy()
        residuals = torch.from_numpy(self.normalizer.fit_transform(residuals)).to(torch.float32).to(self.mlp_model.device)
        train_loader = DataLoader(TensorDataset(self.z1, residuals), batch_size=self.z1.shape[0], shuffle=True)

        val_loader = None
        if self.z1_val is not None:
            residuals_val = self.z2_val - self.linear_model.transform(self.z1_val).detach().cpu().numpy()
            residuals_val = torch.from_numpy(self.normalizer.transform(residuals_val)).to(torch.float32).to(self.mlp_model.device)
            val_loader = DataLoader(TensorDataset(self.z1_val, residuals_val), batch_size=self.z1_val.size(0),
                                    shuffle=False)

        trainer = Trainer(max_epochs=100, min_epochs=5, callbacks=[EarlyStopping(monitor="val_loss", mode="min", patience=10)])
        trainer.fit(self.mlp_model, train_loader, val_loader)
    ...
    def transform(self, z1: Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
        ...
        if isinstance(z1, np.ndarray):
            z1 = torch.from_numpy(z1)
        z2_linear = self.linear_model.transform(z1)
    ...
    @classmethod
    def from_file(cls, path, linear_model=None):
        """
        Loads the results of the optimization problem from a file.
        ...
        instance = cls(linear_model, np.empty((1, 1)), np.empty((1, 1)), 0, 0, dropout=dropout, noise_sigma=noise_sigma)
...
class DecoupleFitting(BaseOptimizer):
    ...
    def from_file(cls, path, mapping_type="Linear"):
        ...
        Parameters:
            path (str): Path to the file containing the results
            mapping_type (str): Type of mapping to load (Linear or Affine)

        Returns:
            DecoupleFitting: Instance of the DecoupleFitting class with the loaded results
        """
        instance = cls(np.empty((1, 1)), np.empty((1, 1)), 0, 0, mapping_type=mapping_type)
